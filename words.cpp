...
const int SENTENCE_MAX_LENGTH = 25;
...
void displayAssociations(std::string word)
{
    ...
    unsigned int totalAssociations = 0;
    unsigned int totalUniqueAssociations = it->second->getAmountOfAssociations();
    std::cout << std::endl;
    for (unsigned int i = 0; i < totalUniqueAssociations; ++i) {
        ...
        if (wordIt == wordMap.end()) {
            ...
            unsigned int totalAssociationsForThisWord = it->second->getNumTimesAssociated(associatedWordID);
            if (i > 0 && i < totalUniqueAssociations) { std::cout << ", "; }
            std::cout << wordIt->second->getWord() << "[" << totalAssociationsForThisWord << "]";
            totalAssociations += totalAssociationsForThisWord;
        }
    }
    std::cout << std::endl << std::endl;
    std::cout << "Total associations: " << totalAssociations << std::endl;
    std::cout << "Total unique associations: " << totalUniqueAssociations << std::endl << std::endl;
}
